...
import { MonitorPlay, Heart, ShoppingCart, Clock, BarChart, Calendar, User, BookOpen, Tag } from 'lucide-react'
...
export default function CourseDetails() {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        const response = await axiosInstance.get(`/user/data/viewcourse/${courseId}`)
      ...
      <div className="relative h-[420px] rounded-2xl overflow-hidden shadow-2xl border border-gray-800"> 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-xl p-6 shadow-lg border border-gray-200"> {/* Changed from blue gradient to white */}
              <h2 className="text-3xl font-bold mb-6 text-gray-900">₹{courseData.price}</h2> {/* Changed text color */}
              <div className="space-y-4">
                <Button className="w-full text-lg py-6 bg-blue-600 text-white hover:bg-blue-700" size="lg">
                  Buy Now
                </Button>
                <Button onClick={addToCart} className="w-full bg-green-500 hover:bg-green-600 text-lg py-6 text-white" size="lg">
                  <ShoppingCart className="w-5 h-5 mr-2" />
                  Add to Cart
                </Button>
                <Button variant="outline" className="w-full text-lg py-6 border-blue-600 text-blue-600 hover:bg-blue-50" size="lg">
